...
class JojContentProvider(ContentProvider):
    ...
    def resolve(self, item, captcha_cb=None, select_cb=None):
        ...
        if url.endswith('live.html'):
            ...
            #print 'clipid = ', clipid
            videophp_url = "https://media.joj.sk/services/Video.php?pageId=%s&clip=%s"% (pageid, clipid.replace('-','%2D'))
            #print 'videophp_url = ', videophp_url
            playlist = fromstring(util.request(videophp_url))
            balanceurl = 'https://media.joj.sk/balance.xml?nc=%d' % random.randint(1000, 9999)
